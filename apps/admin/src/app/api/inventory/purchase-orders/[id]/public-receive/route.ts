...
/**
 * POST /api/inventory/purchase-orders/[id]/public-receive
 * Receive a purchase order (sent → received) without authentication.
 ...
 * Uses "staff" as receivedBy user ID.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Use "staff" as the user ID for public reception
    const userId = 'staff'
    ...
    const receptionPromises = body.items
      ...
      .map(async (item) => {
        ...
        if (priceChanged) {
          ...
          console.log(
            `[Public Receive Order] Updated price for ${productName}: ${orderItem.unitPriceHT} → ${receivedUnitPriceHT} EUR HT`
          )
        }
        ...
        await StockMovement.create({
          ...
          createdBy: userId, // "staff" for public reception
        })
    ...
    order.receivedBy = userId // "staff" for public reception
    if (body.notes) {
    ...
    console.error('[POST /api/inventory/purchase-orders/[id]/public-receive] Error:', error)
    return errorResponse(
