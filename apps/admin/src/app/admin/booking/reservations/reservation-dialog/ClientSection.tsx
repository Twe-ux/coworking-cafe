...
import type { ClientSectionProps, ClientData } from "./types"

export function ClientSection({ selectedClient, onChange, error }: ClientSectionProps) {
  ...
  const handleCreateNewClient = async () => {
    ...
    try {
      ...
      const clientRole = rolesData.data?.find((role: any) => role.slug === "client")
      if (!clientRole) {
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              {selectedClient.company && (
                <p className="text-sm text-muted-foreground font-medium">{selectedClient.company}</p>
              )}
            </div>
            <Button
              type="button"
            ...
            {filteredClients.map((client) => (
              ...
              >
                <div className="flex flex-col">
                  ...
                  {client.company && (
                    <span className="text-xs text-muted-foreground font-medium">{client.company}</span>
                  )}
                </div>
              </button>
            ))}
