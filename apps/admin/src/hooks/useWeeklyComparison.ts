...
export function useWeeklyComparison(
  ...
  return useQuery<WeeklyComparisonData>({
    queryKey: ['weekly-comparison', currentYear, previousYear, mode],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/accounting/analytics/weekly?${params}`);
