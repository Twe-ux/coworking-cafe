...
import { DeleteShiftConfirmDialog } from "./DeleteShiftConfirmDialog";
...
export function DayShiftsModal({
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await onDeleteShift(shiftToDelete.id);
    setDeletingId(null);
    ...
    <>
        ...
        <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-[600px]">
          ...
          <div className="space-y-4 py-4">
            {shifts.length === 0 ? (
              ...
              shifts.map((shift) => {
                const shiftType =
                  SHIFT_TYPES[shift.type as keyof typeof SHIFT_TYPES];
      ...
      <DeleteShiftConfirmDialog
        ...
        shiftInfo={
          shiftToDelete
            ? {
                employeeName: `${shiftToDelete.employee?.firstName} ${shiftToDelete.employee?.lastName}`,
                date: formattedDate,
                time: `${shiftToDelete.startTime} - ${shiftToDelete.endTime}`,
              }
            : undefined
        }
      />
