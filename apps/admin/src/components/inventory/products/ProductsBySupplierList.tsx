...
function SortableSupplierCard({ group, isOpen, onToggle, children }: {
  group: SupplierGroup; isOpen: boolean; onToggle: () => void; children: React.ReactNode
}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: group.supplierId })
  const style = { transform: CSS.Transform.toString(transform), transition }

  return (
    <Card ref={setNodeRef} style={style} className={isDragging ? 'opacity-50 shadow-lg z-50' : ''}>
      <div className="flex items-center gap-2 p-3 hover:bg-green-50 transition-colors">
        <button
...
export function ProductsBySupplierList({
  groups, loading, onEdit, onDeactivate, onReactivate, onReorder,
  ...
  if (groups.length === 0) {
    return (
      <Card>
        <CardContent className="py-8 text-center text-muted-foreground">Aucun produit trouvé</CardContent>
      </Card>
    )
  }
  ...
  if (onReorder) {
    return (
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={groups.map((g) => g.supplierId)} strategy={verticalListSortingStrategy}>
          <div className="space-y-3">
            {groups.map((group) => (
              <SortableSupplierCard
                key={group.supplierId} group={group}
                isOpen={expanded.has(group.supplierId)} onToggle={() => toggle(group.supplierId)}
              >
  ...
  return (
    <div className="space-y-3">
      {groups.map(({ supplierName, supplierId, products }) => {
        ...
        return (
          <Card key={supplierId}>
            <div className="flex items-center gap-3 p-3 hover:bg-green-50 transition-colors cursor-pointer" onClick={() => toggle(supplierId)}>
              {isOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
