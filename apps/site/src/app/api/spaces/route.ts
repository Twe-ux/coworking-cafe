...
export async function GET(request: NextRequest) {
  try {
    ...
    if (minCapacity || maxCapacity) {
      query.capacity = {};
      if (minCapacity) {
        (query.capacity as Record<string, unknown>).$gte =
          parseInt(minCapacity);
      }
      if (maxCapacity) {
        (query.capacity as Record<string, unknown>).$lte =
  ...
  try {
    ...
    if (!name || !description || !type || !capacity) {
      return NextResponse.json(
        {
          success: false,
          error: "Missing required fields: name, description, type, capacity",
        },
        { status: 400 },
      );
    }
    ...
    if (
      !pricing ||
      (!pricing.hourly && !pricing.daily && !pricing.weekly && !pricing.monthly)
    ) {
      return NextResponse.json(
        {
          success: false,
          error:
            "At least one pricing option (hourly, daily, weekly, or monthly) is required",
        },
        { status: 400 },
      );
    }
