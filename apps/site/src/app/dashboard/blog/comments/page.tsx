...
const CommentsPage = () => {
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'approved' | 'rejected' | 'spam'>('pending');
  ...
  const handleApprove = async (id: string, status: 'approved' | 'rejected' | 'spam') => {
    try {
      await approveComment({ id, status }).unwrap();
      success(`Commentaire ${status === 'approved' ? 'approuvé' : status === 'rejected' ? 'rejeté' : 'marqué comme spam'} avec succès`);
    } catch (err: any) {
    ...
    <>
      <Card>
        <CardHeader className="d-flex justify-content-between align-items-center flex-wrap gap-3">
          <CardTitle as="h4" className="mb-0">Gestion des Commentaires</CardTitle>
        ...
        <CardBody>
          {error ? (
            ...
            <div className="text-center py-5">
              <IconifyIcon icon="solar:chat-square-outline" className="fs-48 text-muted mb-3" />
                          ...
                          <div style={{ maxWidth: '250px' }}>
                            ...
                            {comment.parent && (
                              <small className="text-muted">
                                <IconifyIcon icon="solar:reply-outline" className="me-1" />
                                Réponse
                              </small>
                            )}
                          </div>
                        ...
                        <td>
                          <div style={{ maxWidth: '200px' }}>
                            {typeof comment.article === 'object' && comment.article?.title ? (
                              ...
                              <small className="text-muted">Article supprimé</small>
                            )}
                          </div>
                        </td>
                        <td>
                          ...
                          <div className="d-flex gap-1">
                            ...
                            {comment.status === 'pending' && (
                              <>
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => handleApprove(comment._id, 'approved')}
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => handleApprove(comment._id, 'rejected')}
                                  disabled={isApproving}
                                  title="Rejeter"
                                >
                            ...
                            {comment.status !== 'spam' && (
                              <Button
                                ...
                                size="sm"
                                onClick={() => handleApprove(comment._id, 'spam')}
                                disabled={isApproving}
          ...
          {selectedComment && (
            <>
                ...
                <div className="d-flex align-items-center gap-3">
                  ...
                  >
                    <span className="text-primary fw-semibold fs-5">
                      {(selectedComment.user.name || selectedComment.user.username).charAt(0).toUpperCase()}
                    </span>
                  </div>
                  <div>
                    <strong className="d-block">
                      {selectedComment.user.name || selectedComment.user.username}
                    </strong>
                    <small className="text-muted">{selectedComment.user.email}</small>
                  </div>
                </div>
              </div>
              ...
              {typeof selectedComment.article === 'object' && selectedComment.article && (
                <div className="mb-4">
                  ...
                  >
                    ...
                    <IconifyIcon icon="solar:link-circle-outline" className="ms-2" />
                  </Link>
                </div>
              )}
              ...
              <div className="mb-4">
                ...
                <p className="bg-light p-3 rounded">{selectedComment.content}</p>
              </div>
                ...
                <div className="mb-4">
                  ...
                  <p className="bg-light p-3 rounded">
                    <small className="text-muted d-block mb-1">
                      {selectedComment.parent.user.name || selectedComment.parent.user.username}
                    </small>
        ...
        <Modal.Footer>
          {selectedComment && selectedComment.status === 'pending' && (
            <>
              ...
              >
                <IconifyIcon icon="solar:check-circle-outline" className="me-2" />
                Approuver
              </Button>
              <Button
                variant="danger"
              ...
              >
                <IconifyIcon icon="solar:close-circle-outline" className="me-2" />
                Rejeter
              </Button>
            </>
          )}
