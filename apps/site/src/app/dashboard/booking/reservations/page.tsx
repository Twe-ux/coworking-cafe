  ...
  const fetchReservations = async () => {
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "error",
          text: data.error || "Erreur lors du chargement des réservations",
        });
      }
    } catch (error) {
      setMessage({
                ...
                {reservations.length === 0 ? (
                  ...
                  reservations.map((reservation) => (
                    <tr key={reservation._id}>
                      ...
                      <td>
                        <Badge
                          bg={statusColors[reservation.status] || "secondary"}
                        >
                        ...
                        {reservation.attendanceStatus && (
                          <Badge
                            bg={
                              statusColors[reservation.attendanceStatus] ||
                              "secondary"
                            }
                            className="ms-1"
                          >
                            {statusLabels[reservation.attendanceStatus] ||
                              reservation.attendanceStatus}
                          </Badge>
                        )}
                      </td>
