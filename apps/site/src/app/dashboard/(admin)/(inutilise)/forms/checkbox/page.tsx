    ...
    >
      <div className="mt-2">
        {colorVariants.slice(0, 7).map((color, idx) => (
          <div
            className={`form-check form-checkbox-${color} ${
              colorVariants.length - 1 === idx ? "" : "mb-2"
            }`}
            key={idx}
          >
            <input
              ...
              id={`customCheckColor${idx}`}
              defaultChecked
            />
            <label
              className="form-check-label"
              htmlFor={`customCheckColor${idx}`}
            >
...
const ColorsRadio = () => {
  return (
    ...
    >
      {colorVariants.slice(0, 7).map((color, idx) => (
        <div
          className={`form-check form-radio-${color} ${
            colorVariants.length - 1 === idx ? "" : "mb-2"
          }`}
          key={idx}
        >
          <input
            type="radio"
            id={`customRadioColor${idx}`}
            name={`customRadioColor${idx}`}
            className="form-check-input"
            defaultChecked
          />
          <label
            className="form-check-label"
            htmlFor={`customRadioColor${idx}`}
          >
    ...
    <ComponentContainerCard
      ...
      description={
        <>
          ...
          <code>.form-switch</code> class to render a toggle switch. Switches
          alsosupport the <code>disabled</code> attribute.
        </>
      }
    >
      <FormCheck
