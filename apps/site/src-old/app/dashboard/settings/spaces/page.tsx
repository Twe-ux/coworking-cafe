...
export default function SpacesSettingsPage() {
  ...
  const [spaceToDelete, setSpaceToDelete] = useState<SpaceConfiguration | null>(
    null,
  );
  ...
  const fetchConfigurations = async () => {
    try {
      ...
      if (data.success) {
        ...
        setMessage({
          type: "error",
          text: "Erreur lors du chargement des configurations",
        });
      }
    } catch (error) {
      setMessage({
  ...
  const handleSeedData = async () => {
    try {
      ...
      if (response.ok) {
        setMessage({
          type: "success",
          text: "Configurations initiales créées avec succès",
        });
        fetchConfigurations();
      } else {
        setMessage({
  ...
  const handleDeleteSpace = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/space-configurations/${spaceToDelete.spaceType}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleUpdateConfiguration = async (config: SpaceConfiguration) => {
    try {
      setSaving(true);
      const response = await fetch(
        `/api/admin/space-configurations/${config.spaceType}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateConfiguration = (
    spaceType: string,
    updates: Partial<SpaceConfiguration>,
  ) => {
      ...
      prev.map((config) =>
        config.spaceType === spaceType
          ? {
              ...
              pricing: {
                ...config.pricing,
                tiers: config.pricing.tiers?.filter(
                  (_, idx) => idx !== tierIndex,
                ),
              },
            }
  ...
  const getMinPrice = (space: SpaceConfiguration) => {
    ...
    if (space.pricing.tiers && space.pricing.tiers.length > 0) {
      const minHourly = Math.min(
        ...space.pricing.tiers.map((t) => t.hourlyRate).filter((p) => p > 0),
      );
                  ...
                  <Nav variant="tabs" className="mb-3">
                    {configurations.map((config) => (
                      <Nav.Item key={config.spaceType}>
                        <Nav.Link eventKey={config.spaceType}>
                          {config.name}
                        </Nav.Link>
                      </Nav.Item>
                    ))}
                        ...
                        <Form>
                          ...
                          <Form.Group className="mb-3">
                            <Form.Label>
                              Tags / Équipements{" "}
                              <small className="text-muted">
                                (Exemples: WiFi, Café, Écran, Projecteur,
                                Climatisation, Imprimante, Tableau blanc, etc.)
                              </small>
                            </Form.Label>
                              ...
                              <Form.Control
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    ...
                                    if (
                                      value &&
                                      !(config.features || []).includes(value)
                                    ) {
                                      updateConfiguration(config.spaceType, {
                                        features: [
                                          ...(config.features || []),
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const input = document.getElementById(
                                    `feature-input-${config.spaceType}`,
                                  ) as HTMLInputElement;
                                  const value = input?.value.trim();
                                  if (
                                    value &&
                            ...
                            <>
                              ...
                              <div className="mb-4">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                  <h6 className="mb-0">
                                    Tarification par palier (optionnel)
                                  </h6>
                                  <Button
                                    size="sm"
                                    variant="outline-primary"
                                      ...
                                      <Col md={6}>
                                        <Form.Group className="mb-3">
                                          <Form.Check
                                            ...
                                            checked={
                                              config.availableReservationTypes
                                                ?.hourly || false
                                            }
                                            onChange={(e) =>
                                              updateConfiguration(
                                  ...
                                  {config.pricing.perPerson && (
                                    <Row className="mb-3">
                                      <Col md={6}>
                                        <Form.Group>
                                          <Form.Label>
                                            Heures max avant tarif journée
                                            (optionnel)
                                          </Form.Label>
                                          <Form.Control
                                            type="number"
                          ...
                          {config.requiresQuote && (
                            <Alert variant="info">
                              <Icon
                                ...
                                className="me-2"
                              />
                              Mode "sur devis" activé. Les visiteurs verront un
                              message les invitant à vous contacter pour un
                              devis personnalisé.
                            </Alert>
                          )}
                          ...
                          <h5 className="mb-3 mt-4">
                            Politique d'empreinte bancaire
                          </h5>
                          <Form.Group className="mb-3">
                            <Form.Check
                              type="checkbox"
                          ...
                          {config.depositPolicy?.enabled && (
                            <>
                              <Alert variant="info" className="mb-3">
                                ...
                                />
                                <small>
                                  L'empreinte bancaire permet de garantir les
                                  réservations. Elle peut être :
                                  <ul
                                    className="mb-0 mt-2"
                                    style={{ fontSize: "0.85rem" }}
                                  >
                                    <li>
                                      Un <strong>pourcentage</strong> du montant
                                      total
                                    </li>
                                    <li>
                                      Un <strong>montant fixe</strong>
                                    </li>
                                    <li>
                                      Avec un <strong>montant minimum</strong>{" "}
                                      si besoin
                                    </li>
                                  </ul>
                                </small>
                                ...
                                <Col md={6}>
                                  <Form.Group>
                                    ...
                                    <Form.Control
                                      ...
                                      value={
                                        config.depositPolicy?.percentage || ""
                                      }
                                      onChange={(e) =>
                                        updateConfiguration(config.spaceType, {
                                          depositPolicy: {
                                ...
                                <Col md={6}>
                                  <Form.Group>
                                    ...
                                    <Form.Control
                                      ...
                                      placeholder="Ex: 50"
                                      value={
                                        config.depositPolicy?.fixedAmount
                                          ? config.depositPolicy.fixedAmount /
                                            100
                                          : ""
                                      }
                                ...
                                <Col md={6}>
                                  <Form.Group>
                                    ...
                                    <Form.Control
                                      ...
                                      placeholder="Ex: 20"
                                      value={
                                        config.depositPolicy?.minimumAmount
                                          ? config.depositPolicy.minimumAmount /
                                            100
                                          : ""
                                      }
            ...
            <Form>
                ...
                <Col md={6}>
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                      ...
                      onChange={(e) =>
                        setNewSpace({ ...newSpace, spaceType: e.target.value })
                      }
                ...
                <Col md={6}>
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                      ...
                      value={newSpace.slug || ""}
                      onChange={(e) =>
                        setNewSpace({ ...newSpace, slug: e.target.value })
                      }
                      placeholder="ex: salle-reunion"
                    />
                  </Form.Group>
              ...
              <Form.Group className="mb-3">
                <Form.Check
                  ...
                  onChange={(e) =>
                    setNewSpace({ ...newSpace, isActive: e.target.checked })
                  }
                />
              </Form.Group>
