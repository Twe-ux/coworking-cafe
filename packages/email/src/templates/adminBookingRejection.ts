/**
 * Template email : Réservation refusée (Réservation Admin)
 * Couleur : ROUGE (#EF4444)
 *
 * Pour réservations créées par l'admin qui sont refusées
 * (sans mention de libération d'empreinte bancaire)
 */

import { getSpaceDisplayName, getPriceDisclaimerNote } from "./helpers";

interface AdminBookingRejectionData {
  name: string;
  spaceName: string;
  date: string;
  startTime: string;
...
export function generateAdminBookingRejectionEmail(data: AdminBookingRejectionData): string {
