...
/**
 * Script de migration : coworking-admin → coworking_cafe
 *
 * Usage:
 *   MONGODB_URI="mongodb+srv://..." node scripts/migrate-database.js
 *
 * Ce script copie toutes les collections de coworking-admin vers coworking_cafe
 */

const { MongoClient } = require("mongodb");

const SOURCE_DB = "coworking-admin";
const TARGET_DB = "coworking_cafe";
...
async function migrateDatabase() {
  ...
  if (!uri) {
    ...
    console.log(
      'Usage: MONGODB_URI="mongodb+srv://..." node scripts/migrate-database.js',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `📦 ${collections.length} collections trouvées dans ${SOURCE_DB}:\n`,
    );
    ...
    for (const collInfo of collections) {
      ...
      try {
        ...
        if (indexes.length > 1) {
          // > 1 car _id_ est toujours présent
    ...
    console.log(
      `   3. Vérifier le nombre de documents dans chaque collection\n`,
    );
  } catch (error) {
